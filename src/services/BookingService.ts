...
export async function getUserBookings(filters?: {
  ...
  try {
    ...
    const response = await getApiClient().get<Booking[]>(ENDPOINTS.BOOKING.USER_BOOKINGS, options);
 ...
 * @returns 予約一覧
 */
export async function getAttenderBookings(attenderId: string, filters?: {
  status?: 'pending' | 'confirmed' | 'completed' | 'cancelled';
  from?: string;
  to?: string;
}): Promise<Booking[]> {
  try {
    ...
    logApiRequest('GET', ENDPOINTS.BOOKING.ATTENDER_BOOKINGS, { attenderId, ...filters });
    
    const response = await getApiClient().get<Booking[]>(ENDPOINTS.BOOKING.ATTENDER_BOOKINGS, options);
...
export async function getBookingDetails(bookingId: string): Promise<Booking | null> {
  try {
    ...
    const response = await getApiClient().get<Booking>(ENDPOINTS.BOOKING.DETAIL(bookingId));
...
export async function createBooking(bookingData: BookingCreateData): Promise<string> {
  try {
    ...
    const response = await getApiClient().post<{ id: string }>(ENDPOINTS.BOOKING.CREATE, requestData);
...
export async function cancelBooking(bookingId: string, reason?: string): Promise<boolean> {
  try {
    ...
    const response = await getApiClient().patch(ENDPOINTS.BOOKING.CANCEL(bookingId), requestData);
    ...
    // エラーレスポンスの処理
    const errorMessage = response.error?.message || '予約のキャンセルに失敗しました';
    throw new Error(errorMessage);
  } catch (error) {
...
export async function confirmBooking(bookingId: string): Promise<boolean> {
  try {
    ...
    logApiRequest('PATCH', ENDPOINTS.BOOKING.UPDATE_STATUS(bookingId), requestData);
    
    const response = await getApiClient().patch(ENDPOINTS.BOOKING.UPDATE_STATUS(bookingId), requestData);
...
export async function completeBooking(bookingId: string): Promise<boolean> {
  try {
    ...
    // エラーレスポンスの処理
    const errorMessage = response.error?.message || '予約の完了処理に失敗しました';
    throw new Error(errorMessage);
  } catch (error) {
