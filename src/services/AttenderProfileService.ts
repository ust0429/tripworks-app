...
export class AttenderProfileService {
  ...
  static async getCurrentAttenderProfile(): Promise<AttenderProfile | null> {
    try {
      ...
      const response = await enhancedApiClient.client.get<{
        isAttender: boolean;
        attenderId?: string;
      }>(url);
      logApiResponse("GET", url, response);
      ...
      if (response.success && response.data) {
        ...
        // アテンダーの場合はプロフィールを取得
        console.log(
          `Current user is an attender with ID: ${response.data.attenderId}`
        );
  ...
  static async updateProfile(
    ...
    try {
      ...
      const response = await enhancedApiClient.client.put<AttenderProfile>(
        url,
        profile
      );
   ...
   * @returns 更新後のプロフィール
   */
  static async updateProfileFields(
    attenderId: string,
    updateData: any
  ): Promise<AttenderProfile> {
    try {
      ...
      const response = await enhancedApiClient.client.patch<AttenderProfile>(
        url,
        updates
      );
  ...
  static async addExperienceSample(
    ...
    try {
      ...
      const response = await enhancedApiClient.client.post<ExperienceSample>(
        url,
        sampleData
      );
  ...
  static async updateExperienceSample(
    ...
    try {
      console.log(
        `Updating experience sample ${sampleId} for attenderId: ${attenderId}`
      );
      ...
      // API経由で体験サンプルを更新
      const url = `${ENDPOINTS.ATTENDER.DETAIL(
        attenderId
      )}/experiences/${sampleId}`;
      logApiRequest("PATCH", url, updateData);

      const response = await enhancedApiClient.client.patch<ExperienceSample>(
        url,
        updateData
      );
      logApiResponse("PATCH", url, response);
      ...
      const sampleIndex = profile.experienceSamples.findIndex(
        (s) => s.id === sampleId
      );
      if (sampleIndex === -1) {
   ...
   * @returns 成功したかどうか
   */
  static async removeExperienceSample(
    attenderId: string,
    sampleId: string
  ): Promise<boolean> {
    try {
      console.log(
        `Removing experience sample ${sampleId} for attenderId: ${attenderId}`
      );
  ...
  static async updateAvailability(
    ...
    try {
      ...
      const response = await enhancedApiClient.client.put(url, {
        availability,
      });
      logApiResponse("PUT", url, response);
  ...
  static async updateProfileImage(
    ...
    try {
      ...
      const response = await enhancedApiClient.client.uploadFile<{
        imageUrl: string;
      }>(url, imageFile, "image", { attenderId }, {}, progressCallback);
      logApiResponse("POST", url, response);
   ...
   * @returns 成功した場合は画像URL
   */
  static async addExperienceSampleImage(
    attenderId: string,
    sampleId: string,
    imageFile: File
  ): Promise<string> {
  ...
  private static createSampleProfile(attenderId: string): AttenderProfile {
    ...
    return {
      ...
      background:
        "東京出身。アートギャラリーで3年間働いた経験があり、芸術分野に知見があります。",
      experienceSamples: [
        {
          id: "sample_1",
          title: "下町アートギャラリー巡り",
          description:
            "東京の下町にある小さなギャラリーを巡るツアーです。地元アーティストの作品を見ながら、文化的背景も解説します。",
          category: "アート",
          subcategory: "ギャラリー",
