...
const MOCK_REVIEWS: Record<string, Review> = {
  rev_123: {
    ...
    rating: 5,
    comment:
      "素晴らしい体験でした！アテンダーの方の知識が豊富で、陶芸の奥深さを知ることができました。",
  ...
  rev_124: {
    ...
    rating: 4,
    comment:
      "良い体験でした。もう少し時間があれば完成度の高い作品ができたかもしれません。",
    helpfulCount: 5,
    replyCount: 1,
 ...
 * @returns レビュー一覧
 */
export async function getAttenderReviews(
  attenderId: string,
  limit: number = 10
): Promise<Review[]> {
  try {
    ...
    logApiRequest("GET", ENDPOINTS.REVIEW.ATTENDER_REVIEWS(attenderId), {
      limit,
    });

    const response = await getApiClient().get<Review[]>(
...
export async function getExperienceReviews(
  ...
  try {
    ...
    logApiRequest("GET", ENDPOINTS.REVIEW.EXPERIENCE_REVIEWS(experienceId), {
      limit,
    });
...
export async function getReviewSummary(
  ...
  try {
    ...
    logApiRequest(
      "GET",
      `${ENDPOINTS.REVIEW.ATTENDER_REVIEWS(attenderId)}/summary`,
      {}
    );

    const response = await getApiClient().get<ReviewSummary>(
      `${ENDPOINTS.REVIEW.ATTENDER_REVIEWS(attenderId)}/summary`
    );
...
export async function createReview(
  ...
  try {
    ...
    logApiRequest("POST", ENDPOINTS.REVIEW.CREATE, {
    ...
    // エラーレスポンスの処理
    const errorMessage =
      response.error?.message || "レビューの投稿に失敗しました";
...
export async function updateReview(
  ...
  try {
    ...
    // エラーレスポンスの処理
    const errorMessage =
      response.error?.message || "レビューの更新に失敗しました";
    throw new Error(errorMessage);
  } catch (error) {
...
export async function deleteReview(reviewId: string): Promise<boolean> {
  try {
    ...
    // エラーレスポンスの処理
    const errorMessage =
      response.error?.message || "レビューの削除に失敗しました";
    throw new Error(errorMessage);
  } catch (error) {
...
export async function markReviewAsHelpful(reviewId: string): Promise<boolean> {
  try {
    ...
    // エラーレスポンスの処理
    const errorMessage =
      response.error?.message || "「役に立った」の追加に失敗しました";
    throw new Error(errorMessage);
  } catch (error) {
