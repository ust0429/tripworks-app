...
export async function getReviewDetails(
  ...
  try {
    ...
    const response = await api.get<Review>(ENDPOINTS.REVIEW.DETAIL(reviewId));
...
export async function updateReview(
  ...
  try {
    ...
    const response = await api.put(ENDPOINTS.REVIEW.UPDATE(reviewId), {
...
export async function deleteReview(reviewId: string): Promise<boolean> {
  try {
    ...
    const response = await api.delete(ENDPOINTS.REVIEW.DELETE(reviewId));
...
export async function markReviewAsHelpful(reviewId: string): Promise<boolean> {
  try {
    ...
    // 本番環境ではAPIを使用
    logApiRequest("POST", ENDPOINTS.REVIEW.DETAIL(reviewId) + "/helpful", {
      userId,
    });
