...
const calculateProfileCompletionScore = (profile: AttenderProfile): number => {
  ...
  if (Array.isArray(profile.expertise) && profile.expertise.length > 0) {
    ...
    // 専門分野の詳細
    const expertiseDetails = profile.expertise.map(exp => [
      !!exp.category,
    ...
    score += expertiseDetails.filter(Boolean).length / (expertiseDetails.length || 1);
  }
  totalFields += 1;
  
  // 体験サンプル
  if (Array.isArray(profile.experienceSamples) && profile.experienceSamples.length > 0) {
    score += 1;
    
    // 体験サンプルの詳細
    const sampleDetails = profile.experienceSamples.map(sample => [
      !!sample.title,
      !!sample.description,
      !!sample.category,
      !!sample.estimatedDuration || !!sample.duration,
      !!sample.price || !!sample.pricePerPerson,
      Array.isArray(sample.images) && sample.images.length > 0 || !!sample.imageUrl
    ]).flat();
  ...
  // 利用可能時間
  if (
    (Array.isArray(profile.availableTimes) && profile.availableTimes.length > 0) ||
  ...
  // SNSリンク
  if (profile.socialMediaLinks && Object.values(profile.socialMediaLinks).some(Boolean)) {
...
const generateProfileImprovementTips = (profile: AttenderProfile): string[] => {
  ...
  if (!profile.bio && !profile.biography) {
    ...
    tips.push('自己紹介文をより詳しく書くと、訪問者にあなたの人柄が伝わりやすくなります。');
  }
  ...
  if (!Array.isArray(profile.expertise) || profile.expertise.length === 0) {
    ...
    const incompleteExpertise = profile.expertise.filter(exp => 
      !exp.description || !exp.yearsOfExperience || !Array.isArray(exp.subcategories) || exp.subcategories.length === 0
    );
    
    if (incompleteExpertise.length > 0) {
      tips.push('専門分野の詳細情報（経験年数、説明、サブカテゴリ）を充実させましょう。');
    }
  }
  
  // 体験サンプル
  if (!Array.isArray(profile.experienceSamples) || profile.experienceSamples.length === 0) {
    tips.push('体験サンプルを追加して、どのような体験が提供できるか具体的に示しましょう。');
  } else if (profile.experienceSamples.length < 2) {
    tips.push('複数の体験サンプルを登録すると、より多くの訪問者の興味を引くことができます。');
  } else {
    const samplesWithoutImages = profile.experienceSamples.filter(sample => 
      (!Array.isArray(sample.images) || sample.images.length === 0) && !sample.imageUrl
    );
  ...
  // SNSリンク
  if (!profile.socialMediaLinks || Object.values(profile.socialMediaLinks).filter(Boolean).length === 0) {
    tips.push('SNSアカウントやウェブサイトを登録すると、あなたの活動をより詳しく知ってもらえます。');
  }
...
// プロバイダーコンポーネント
export const AttenderProfileProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const loadProfile = async (id: string): Promise<void> => {
    try {
      ...
      // APIからプロフィール取得
      const response = await apiClient.get<AttenderProfile>(ENDPOINTS.ATTENDER.DETAIL(id));
      ...
      if (response.success && response.data) {
        ...
        setState({
          ...state,
          loadingState: 'failed',
          error: response.error?.message || 'プロフィールの読み込みに失敗しました'
        });
      }
    } catch (err) {
