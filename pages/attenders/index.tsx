    ...
    const fetchAttenders = async () => {
      try {
        ...
        if (response.success && Array.isArray(response.data)) {
          ...
          setError(response.error?.message || 'アテンダー情報の取得に失敗しました');
        }
      } catch (err) {
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label htmlFor="location" className="block text-gray-700 mb-1">場所</label>
              <input
                type="text"
                id="location"
                name="location"
            ...
            <div>
              <label htmlFor="specialty" className="block text-gray-700 mb-1">専門分野</label>
              <input
                type="text"
                id="specialty"
                name="specialty"
            ...
            <div>
              <label htmlFor="rating" className="block text-gray-700 mb-1">最低評価</label>
              <select
                id="rating"
                name="rating"
        ...
        {!loading && !error && (
          <>
            {attenders.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-gray-600">検索条件に合うアテンダーが見つかりませんでした</p>
              </div>
            ) : (
                    ...
                    <a className="block border rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                      <div className="h-48 bg-gray-200 relative">
                        ...
                        {(attender.averageRating || attender.rating) && (
                          <div className="absolute bottom-0 left-0 bg-black bg-opacity-70 text-white px-3 py-1 flex items-center">
                            ...
                            <span className="ml-1">
                              {(attender.averageRating || attender.rating).toFixed(1)}
                            </span>
                      ...
                      <div className="p-4">
                        ...
                        <div className="mt-2 flex flex-wrap gap-1">
                          {attender.specialties.slice(0, 3).map((specialty, index) => (
                            <span 
                              key={index} 
                        ...
                        <div className="mt-2 text-sm text-gray-500 line-clamp-2">
                          {attender.bio || attender.biography || '自己紹介はまだ設定されていません。'}
                        </div>
                      </div>
                    </a>
                  </Link>
