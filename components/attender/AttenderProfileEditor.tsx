...
const AttenderProfileEditor: React.FC<AttenderProfileEditorProps> = ({
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          setError(
            response.error?.message || "プロフィール情報の取得に失敗しました"
          );
        }
      } catch (err) {
  ...
  const handleRemoveSubcategory = (
    ...
    updatedExpertise[expertiseIndex].subcategories =
      updatedExpertise[expertiseIndex].subcategories?.filter(
            ...
            <div key={expIndex} className="mb-4 p-4 border rounded-md">
              <div className="mb-2">
                ...
                <input
                  ...
                  value={exp.category}
                  onChange={(e) =>
                    handleExpertiseChange(expIndex, "category", e.target.value)
                  }
                  className="w-full px-3 py-2 border rounded-md"
                  required
                />
                  ...
                  {exp.subcategories?.map((subcategory, scIndex) => (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          handleRemoveSubcategory(expIndex, subcategory)
                        }
                        className="ml-2 text-indigo-500 hover:text-indigo-700"
                      >
                        ×
