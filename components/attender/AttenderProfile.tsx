...
const AttenderProfileDisplay: React.FC<AttenderProfileProps> = ({ attenderId }) => {
  ...
  if (!profile) {
    return <div className="p-4 text-center">アテンダー情報が見つかりません</div>;
  }
        ...
        <div className="md:w-2/3 p-2">
          ...
          <div className="mt-4">
            ...
            <p className="mt-2 text-gray-700 whitespace-pre-line">
              {profile.bio || profile.biography || '自己紹介はまだ設定されていません。'}
            </p>
          </div>
            ...
            <div className="mt-2 space-y-1">
              {profile.languages.map((lang, index) => (
                <div key={index} className="flex items-center">
                  ...
                  <span className="ml-2 text-sm text-gray-500">
                    ({lang.proficiency === 'native' ? 'ネイティブ' : 
                      lang.proficiency === 'advanced' ? '上級' :
                      lang.proficiency === 'intermediate' ? '中級' : '初級'})
                  </span>
                </div>
              ))}
            </div>
