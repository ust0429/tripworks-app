...
import apiClient, { logApiRequest, logApiResponse } from '../../utils/apiClientEnhanced';
...
// 体験カテゴリの選択肢
const CATEGORIES = [
  '伝統文化', 'アート', '食文化', 'クラフト', 'アウトドア', 
  '音楽', 'ローカルツアー', 'ワークショップ', 'その他'
];
...
const ExperienceForm: React.FC<ExperienceFormProps> = ({
  ...
  const [formData, setFormData] = useState<Partial<ExperienceSample>>(initialFormData);
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          setFormData({
            ...
            estimatedDuration: response.data.estimatedDuration || response.data.duration || 60,
    ...
    for (let i = 0; i < imageFiles.length; i++) {
      ...
      try {
        ...
        const updateProgress = (fileProgress: number) => {
          const totalProgress = (i * progressPerFile) + (fileProgress * progressPerFile / 100);
        ...
        if (response.success && response.data?.url) {
          ...
          console.error(`画像「${file.name}」のアップロードに失敗:`, response.error);
        }
      } catch (err) {
        console.error(`画像「${file.name}」のアップロード中にエラーが発生:`, err);
      }
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="bg-white p-6 rounded-lg shadow-sm">
        ...
        {/* カテゴリ */}
        <div className="mb-4">
          <label htmlFor="category" className="block text-gray-700 font-medium mb-2">カテゴリ</label>
          <select
            id="category"
            name="category"
        ...
        {/* サブカテゴリ */}
        <div className="mb-4">
          <label htmlFor="subcategory" className="block text-gray-700 font-medium mb-2">サブカテゴリ (任意)</label>
          <input
            type="text"
            id="subcategory"
            name="subcategory"
        ...
        {/* 説明 */}
        <div className="mb-4">
          <label htmlFor="description" className="block text-gray-700 font-medium mb-2">説明文</label>
          <textarea
            id="description"
            name="description"
        ...
        {/* 所要時間 */}
        <div className="mb-4">
          <label htmlFor="estimatedDuration" className="block text-gray-700 font-medium mb-2">所要時間 (分)</label>
          <input
            type="number"
        ...
        {/* 価格 */}
        <div className="mb-4">
          <label htmlFor="price" className="block text-gray-700 font-medium mb-2">価格 (円)</label>
          <input
            type="number"
            id="price"
            name="price"
        ...
        {/* 場所 */}
        <div className="mb-4">
          <label htmlFor="location" className="block text-gray-700 font-medium mb-2">開催場所</label>
          <input
            type="text"
            id="location"
            name="location"
        ...
        <div className="mb-4">
          ...
          {formData.images && formData.images.length > 0 && (
            <div className="mb-4">
              <p className="text-sm text-gray-600 mb-2">アップロード済み画像:</p>
        ...
        {loading && uploadProgress > 0 && (
          <div className="mb-4">
            ...
            <p className="text-sm text-gray-600 mt-1">画像アップロード中... {uploadProgress}%</p>
          </div>
        )}
